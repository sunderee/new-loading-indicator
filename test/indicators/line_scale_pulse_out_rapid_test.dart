...
import 'package:new_loading_indicator/src/indicators/line_scale_pulse_out_rapid.dart';
import 'package:new_loading_indicator/src/shape/indicator_painter.dart';

void main() {
  group('LineScalePulseOutRapid', () {
    testWidgets('renders correct number of shapes and transitions', (
        ...
        MaterialApp(
          home: DecorateContext(
            decorateData: DecorateData(
              indicator: Indicator.lineScalePulseOutRapid,
              colors: const [Colors.blue],
              pause: false,
            ),
            child: const LineScalePulseOutRapid(),
          ),
        ),
      );
      ...
      // Should find 5 FractionallySizedBox widgets
      expect(find.byType(FractionallySizedBox), findsNWidgets(5));

      // Should find 6 AnimatedBuilder widgets (5 for lines + 1 from MaterialApp)
      expect(find.byType(AnimatedBuilder), findsNWidgets(6));

      // Should find 9 Expanded widgets (5 for lines + 4 for spacers)
      expect(find.byType(Expanded), findsNWidgets(9));
    ...
    testWidgets('animations are properly configured with delays', (
      ...
      for (final heightFactor in longerDelayHeightFactors) {
        expect(
          heightFactor! >= 0.3 && heightFactor <= 1.0,
          isTrue,
          reason: 'Height factor should stay between 0.3 and 1.0',
        );
      }

      // Verify that lines have different heights due to different delays
      ...
      expect(
        ...
        greaterThan(1),
        reason: 'Lines should have different heights due to different delays',
      );

      // Verify center line (index 2) starts animating first
      expect(
        longerDelayHeightFactors[2] != initialHeightFactors[2],
        isTrue,
        reason: 'Center line should start animating first (no delay)',
      );
    });
